...
- Fonts in `main/ui/fonts/` are generated by `tools/gen_font.py`. Never
  ...
  `main/ui/font.h`. It takes a face name, not a point size: the `FACES` table
  fixes each face's cap height, box, baseline and pitch across every family,
  which is what lets `text_font` switch families without moving the layout.
  A new family goes into `Family` and `kFamilies` in the same order, and into
  `kFonts` in `main/app/settings.cpp`. If its ink does not fit the boxes the
  generator refuses to emit the glyph, and widening a box means rebaking every
  family and rechecking the lines-per-screen table in `docs/design.md`.
- `main/ui/icons_data.h` is generated by `tools/gen_icons.py` the same way,
